...
const TicketDetail = () => {
  ...
  const statusBtns = [
    { value: 'PENDING', label: 'Pending', active: 'border-orange-400 text-orange-600 bg-orange-50 dark:border-orange-500/50 dark:text-orange-400 dark:bg-orange-500/10' },
    { value: 'UNRESOLVED', label: 'Unresolved', active: 'border-red-400 text-red-500 bg-red-50 dark:border-red-500/50 dark:text-red-400 dark:bg-red-500/10' },
    ...
    return (
      <div className="flex h-screen bg-gray-50 dark:bg-[#1b1b1b]">
        ...
        <div className="flex-1 flex items-center justify-center">
          <p className="text-sm text-gray-400 dark:text-gray-500">Loading ticket...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="bg-white dark:bg-[#1b1b1b] border-b border-gray-200 dark:border-gray-800 px-6 py-3 flex items-center gap-3 flex-shrink-0">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {slaBreached && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-red-50 text-red-500 border border-red-200 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/30">
                ⏰ SLA Breach
              </span>
            )}
          </div>
        ...
        <div className="flex-1 overflow-auto p-6">
          {/* SLA alert */}
          {slaBreached && (
            <div className="bg-red-50 border border-red-200 rounded-lg px-4 py-3 flex items-center gap-3 text-sm text-red-600 mb-5 dark:bg-red-500/10 dark:border-red-500/30 dark:text-red-400">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className="lg:col-span-2 flex flex-col gap-5">
              ...
              <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-700 rounded-xl p-5">
                ...
                <h3 className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-4">Issue details</h3>
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <p className="text-xs text-gray-400 dark:text-gray-500 mb-0.5">Issue type</p>
                    <p className="text-sm text-gray-900 dark:text-gray-100">{ticket.issueType}</p>
                  </div>
                  <div>
                ...
                {ticket.customIssue && (
                  <div className="mb-4">
                    <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">Custom issue</p>
                    <p className="text-sm text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-white/5 rounded-lg px-3 py-2 break-words whitespace-pre-wrap overflow-hidden">
              ...
              {ticket.attachment && (
                <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-700 rounded-xl p-5">
                  <h3 className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-4">Attachment</h3>
                  <a
                    href={ticket.attachment}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-3 border border-gray-200 dark:border-gray-700 rounded-lg px-4 py-3 hover:bg-blue-50 dark:hover:bg-blue-500/10 hover:border-blue-200 dark:hover:border-blue-500/30 transition-all"
                  >
                    <div className="w-9 h-9 bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/30 rounded-lg flex items-center justify-center text-lg flex-shrink-0">
                      📎
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-800 dark:text-gray-200 truncate">View attachment</p>
                      <p className="text-xs text-gray-400 dark:text-gray-500">Click to open in new tab</p>
                    </div>
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={i} className="flex items-start gap-3">
                      ...
                      <div>
                        <p className={`text-sm ${item.red ? 'text-red-500 dark:text-red-400' : 'text-gray-700 dark:text-gray-300'}`}>{item.text}</p>
                        {item.time && <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{formatDateTime(item.time)}</p>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="flex flex-col gap-5">
              <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-700 rounded-xl p-5">
                <h3 className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-4">Ticket actions</h3>

                {/* Status */}
                <p className="text-xs text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-2">Status</p>
                  ...
                  {statusBtns.map(btn => (
                    <button
                      ...
                      className={`py-2 rounded-lg border text-xs font-medium transition-all ${
                        status === btn.value ? btn.active : 'border-gray-200 dark:border-gray-700 text-gray-400 dark:text-gray-500 bg-white dark:bg-white/5 hover:border-gray-300 dark:hover:border-gray-600'
                      }`}
                    >
                      {btn.label}
                    </button>
                ...
                {user.role === 'SUPER_ADMIN' && (
                  <>
                    <p className="text-xs text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-2">Assigned to</p>
                    <select
                      value={assignedTo}
                      onChange={e => handleAssign(e.target.value)}
                      className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-white/5 text-sm text-gray-700 dark:text-gray-300 focus:outline-none focus:border-blue-500 dark:focus:border-blue-500 mb-4"
                    >
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-1">
                  Internal note <span className="normal-case text-gray-300 dark:text-gray-600">(admin only)</span>
                </p>
                {internalNote && (
                  <div className="bg-gray-50 dark:bg-white/5 border-l-2 border-gray-200 dark:border-gray-700 rounded-r-lg px-3 py-2 mb-2 text-xs text-gray-600 dark:text-gray-400 leading-relaxed whitespace-pre-wrap">
                    {internalNote}
                  </div>
                )}
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-white/5 text-sm text-gray-700 dark:text-gray-300 focus:outline-none focus:border-gray-400 dark:focus:border-gray-500 resize-none mb-1"
                />
                <p className="text-xs text-gray-300 dark:text-gray-600 mb-4">Only visible to admins</p>

                <hr className="border-gray-100 dark:border-gray-800 mb-4" />

                {/* Remark */}
                <p className="text-xs text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-1">
                  Remark for submitter <span className="normal-case text-gray-300 dark:text-gray-600">(optional)</span>
                </p>
                <textarea
                  value={remark}
                  ...
                  rows={3}
                  className="w-full px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-white/5 text-sm text-gray-700 dark:text-gray-300 focus:outline-none focus:border-yellow-400 dark:focus:border-yellow-500 resize-none mb-1"
                />
                <p className="text-xs text-gray-300 dark:text-gray-600 mb-4">Shown to submitter when resolved</p>
